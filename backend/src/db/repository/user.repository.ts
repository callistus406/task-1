...
export const findUserAndWallet = async (email: string) => {
...
export const findUserProfile = async (userId: mongooseType) => {
  const response = await UserModel.findOne({ _id: userId }).select(
    "-refresh_token -otp_secrete -createdAt -updatedAt -hashed_password"
  );

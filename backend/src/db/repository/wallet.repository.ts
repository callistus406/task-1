...
export const findUserWallet = async (walletId: mongooseType) => {
    try {
      const response = await WalletModel.findById(walletId).populate("user", "first_name email last_name"); 
...
export const findWalletByUser = async (userId: mongooseType) => {
    try {
      const response = await WalletModel.findOne({user: userId}).populate("user", "first_name email last_name"); 

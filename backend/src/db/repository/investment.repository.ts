...
export const createInvestment = async (
  ...
  const response = await InvestmentPlanModel.create({
    ...data,
    createdBy: userId,
  });
...
export const updateInvestmentPlan = async (
  ...
  const response = await InvestmentPlanModel.updateOne(
    { _id: investmentId },
    { $set: { ...data } },
    { new: true }
  );
...
export const getInvestmentTotals = async () => {
  return await InvestmentPlanModel.aggregate([
    {
      $group: {
        _id: "$investmentPlan",

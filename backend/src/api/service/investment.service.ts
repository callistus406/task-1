...
import { createInvestment, deleteInvestmentPlan, findInvestmentPlan, findInvestments, updateInvestmentPlan } from '../../db/repository/investment.repository';
import { mongooseType } from '../../@types/express';
import { createSubscription } from '../../db/repository/subscription.respositry';
import { isValidObjectId } from 'mongoose';


export const createInvestmentService = async (data:IInvestmentPlan,userId: mongooseType) => {
...
export const updateInvestmentPlanService = async (data:IUpdateInvestmentPlan,investmentId: string) => {
    if (!investmentId || !isValidObjectId(investmentId)) {
        throw createCustomError("Invalid transaction ID.", 422);
      } 
